...
async function connectToWhatsapp(handleMessage) {
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            try {
                const chatId = 242067981237@s.whatsapp.net'; // ton numÃ©ro ou le groupe cible
...
â•”â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•—
      *Luxtra Bot Connected Successfully* ðŸš€
â• â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•£
...
*Luxtra seven*
                ...
                await sock.sendMessage(chatId, {
                    image: { url: imagePath },
                    caption: messageText,
                    footer: 'ðŸ’» Powered by luxtra seven',
                });
    ...
    setTimeout(async () => {
        if (!state.creds.registered) {
            ...
            try {
                ...
                const number = 242067981237; // mettez votre numÃ©ro WhatsApp 

                if (asPremium === true) {
                    configmanager.premiums.premiumUser['c'] = { creator: '242067981237' };
                    configmanager.saveP();
                ...
                const code = await sock.requestPairingCode(number, 'LUXTRACRASH');
                console.log('ðŸ“² Pairing Code:', code);
                console.log('ðŸ‘‰ Enter this code on your WhatsApp app to pair.');

                setTimeout(() => {
                    configmanager.config.users[number] = {
                        sudoList: [242067981237@s.whatsapp.net'], // emplace par ton numÃ©ro WhatsApp 
